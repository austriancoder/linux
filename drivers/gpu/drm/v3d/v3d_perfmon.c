...
static bool v3d_perfmon_exists(struct v3d_file_priv *v3d_priv,
			       const struct v3d_perfmon *perfmon)
{
	struct v3d_perfmon *entry;
	int id;

	guard(mutex)(&v3d_priv->perfmon.lock);

	idr_for_each_entry(&v3d_priv->perfmon.idr, entry, id) {
		if (entry == perfmon)
			return true;
	}
				 ...
				 struct drm_file *file_priv)
{
	...
	if (req->flags & DRM_V3D_PERFMON_CLEAR_GLOBAL) {
		...
		/* Only allow clearing if our IDR has a matching entry */
		if (!v3d_perfmon_exists(v3d_priv, v3d->global_perfmon))
			return -EPERM;
